...
from model.recycling.modeling_qwen_kv import Qwen2ForCausalLM
...
@torch.no_grad()
def recycling_forward(inputs, model, tokenizer, max_new_tokens, output_id_topk=8, tree_version="2.2.2"):
    ...
    max_cache_len = start_len + max_new_tokens + len(tree_template) + 16
    (
        past_key_values,
    ...
    if model_eos is not None:
        if isinstance(model_eos, (list, tuple)):
            eos_token_ids.extend(model_eos)
        else:
            eos_token_ids.append(model_eos)
    eos_token_ids = torch.tensor(
        sorted(set(int(x) for x in eos_token_ids)),
        dtype=torch.long,
        device=torch.device("cuda:0"),
    )

    stop_token_ids = eos_token_ids.tolist()
    start_header_id = tokenizer.convert_tokens_to_ids("<|start_header_id|>")
    if start_header_id is not None and start_header_id >= 0:
        stop_token_ids.append(int(start_header_id))
    stop_token_ids = torch.tensor(sorted(set(stop_token_ids)), dtype=torch.long, device=torch.device("cuda:0"))

    def has_eos(token_tensor: torch.Tensor) -> bool:
        if eos_token_ids.numel() == 0:
            return False
        return torch.isin(token_tensor, eos_token_ids).any().item()
    ...
    for i in range(max_new_tokens):
        ...
        merge_attention_mask = torch.cat(
            [
                torch.ones(
                    [input_ids.size(0), len(tree_template) + 1, verify_input_ids.size(1)],
                    dtype=torch.bool,
                    device=torch.device("cuda:0"),
                ),
                attention_mask.bool(),
        ...
        if has_eos(best_path_input) or verify_input_ids.size(1) - start_len > max_new_tokens:
            break
        ...
        if has_eos(input_ids):
            start_len -= 1
            break

        step += 1
    # Speculative verification may append several tokens at once. Truncate generated tail
    # at the first stop marker to avoid leaking next-turn assistant headers.
    gen_tokens = verify_input_ids[:, start_len:]
    if gen_tokens.numel() > 0 and stop_token_ids.numel() > 0:
        stop_mask = torch.isin(gen_tokens[0], stop_token_ids)
        stop_pos = torch.nonzero(stop_mask, as_tuple=False)
        if stop_pos.numel() > 0:
            first_stop_idx = int(stop_pos[0].item())
            original_len = verify_input_ids.size(1)
            verify_input_ids = verify_input_ids[:, : start_len + first_stop_idx]
...
if __name__ == "__main__":
    ...
    if model_config.model_type == "llama":
        ...
        raise ValueError(f"Unsupported model_type for recycling: {model_config.model_type}")
