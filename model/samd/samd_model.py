...
class SamdModel(nn.Module):
    ...
    def __init__(self,
        ...
        dtype: torch.dtype,
        device: str,
        stop_token_id: Optional[Union[int, List[int], Tuple[int, ...]]] = None,
    ) -> None:
        super().__init__()
        self.samd_config = samd_config
        self.gen_config: SamdGenerationConfig = None
        self.eos_tokens = self._normalize_token_ids(eos_token_id)
        self.stop_tokens = self._normalize_token_ids(stop_token_id)
    ...
    @staticmethod
    def _normalize_token_ids(token_ids: Optional[Union[int, List[int], Tuple[int, ...]]]) -> List[int]:
    ...
    def _find_first_stop(self, new_ids: List[int]) -> Optional[int]:
        stop_set = set(self.eos_tokens + self.stop_tokens)
    ...
    def decode(
        ...
        length: int,
        remaining_cache_len: Optional[int] = None,
    ):
        ...
        max_cache_len = None
        current_cache_len = 0
        if self.cache is not None and hasattr(self.cache, "get_max_cache_shape"):
            max_cache_len = self.cache.get_max_cache_shape()
        if self.cache is not None and hasattr(self.cache, "get_seq_length"):
            current_cache_len = self.cache.get_seq_length()
        actual_remaining_cache_len = None
        if max_cache_len is not None:
            actual_remaining_cache_len = max_cache_len - current_cache_len
        if remaining_cache_len is None:
            remaining_cache_len = actual_remaining_cache_len
        elif actual_remaining_cache_len is not None:
            remaining_cache_len = min(remaining_cache_len, actual_remaining_cache_len)
        if remaining_cache_len is not None and input_ids.shape[1] > remaining_cache_len:
            return sample_p, []
        if max_cache_len is not None and position_ids is not None:
    ...
    def generate(self,
        ...
        for step in range(generation_config.max_new_tokens):
            current_cache_len = self.cache.get_seq_length() if self.cache is not None else (input_length + decode_tokens)
            remaining_cache_len = generation_config.max_cache_len - current_cache_len
            if remaining_cache_len <= 0:
                break
            sample_p, new_ids = self.decode(
                sample_p,
                input_length + decode_tokens,
                remaining_cache_len=remaining_cache_len,
            )
            if len(new_ids) == 0:
                break
            eos_index = self._find_first_stop(new_ids)
